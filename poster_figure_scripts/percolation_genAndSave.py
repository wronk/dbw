...
def construct_graph_list(graphs_to_const):
    ...
    if graph_check[3] in graphs_to_const:
        '''
        G_BIO, _, _ = bp_und(N=bc.num_brain_nodes,
...
und_rand = np.zeros((len(graph_names), len(func_list), len(prop_rm),
                     repeats))
und_targ = np.zeros((len(graph_names), len(func_list),
                     len(lesion_list), repeats))

# Matrices for directed random and targetted attacks
...
for ri in np.arange(repeats):
    ...
    for gi, G in enumerate(graph_list):
        ...
        for fi, func in enumerate(func_list):
            und_rand[gi, fi, :, ri] = perc.percolate_random(G, prop_rm, func)
            und_targ[gi, fi, :, ri] = perc.percolate_degree(G, lesion_list,
                                                            func)
        print ' ... Done'
...
for gi, G in enumerate(graph_list):
    ...
    pickle.dump({'graph_name': graph_names[gi], 'metrics_list':
                 ...
                 'data_target': und_rand[gi, :, :, :],
                 'data_rand': und_targ[gi, :, :, :]}, outfile)
