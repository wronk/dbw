...
# Get in & out degree
indeg_attachment = np.array([Gattachment.in_degree()[node] for node in Gattachment])
outdeg_attachment = np.array([Gattachment.out_degree()[node] for node in Gattachment])
...
ax0.scatter(indeg_attachment, outdeg_attachment, c=ATTACHMENTCOLOR, s=cf.MARKERSIZE, lw=0)
...
ax0_histTop.hist(indeg_attachment, bins=cf.OUTDEGREE_BINS, histtype='stepfilled', color=ATTACHMENTCOLOR, alpha=.6)
ax0_histRight.hist(outdeg_attachment, bins=cf.OUTDEGREE_BINS, orientation='horizontal', histtype='stepfilled', color=ATTACHMENTCOLOR, alpha=.6)

ax0_histTop.hist(indeg_growth, bins=cf.OUTDEGREE_BINS, histtype='stepfilled', color=GROWTHCOLOR, alpha=.6)
ax0_histRight.hist(outdeg_growth, bins=cf.OUTDEGREE_BINS, orientation='horizontal', histtype='stepfilled', color=GROWTHCOLOR, alpha=.6)

plt.setp(ax0_histTop.get_xticklabels() + ax0_histRight.get_yticklabels(),
             visible=False)
             
ax0_histTop.set_ylabel('#\nNodes')
...
ax1.scatter(deg_attachment, percent_indeg_attachment, s=cf.MARKERSIZE, lw=0, c=ATTACHMENTCOLOR)
ax1.scatter(deg_growth, percent_indeg_growth, s=cf.MARKERSIZE, lw=0, c=GROWTHCOLOR)
