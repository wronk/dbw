...
for graph_idx in range(N_WS_GRAPHS):
    G_WS = nx.watts_strogatz_graph(n_nodes,37,0.159)
...
for graph_idx in range(N_ER_GRAPHS):
    G_BA = nx.barabasi_albert_graph(n_nodes, 19)
...
axs_degvcc[0,0].scatter(clustering, degree)
axs_degvcc[0,1].scatter(ER_clustering, ER_degree)
axs_degvcc[1,0].scatter(WS_clustering, WS_degree)
